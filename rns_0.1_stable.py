...
# 品”）的个人和法人实体授权：
# 被授权个人或法人实体有权以任何目的处置授权作品，包括但不限于使用、复制，修改（协议允
# 许修改的受限部分），衍生利用、散布，发布和再许可：
...
# 5.禁止在分发版本中删除或修改此许可证或明示（或暗示）虚假的授权作品原作者信息。
# 6.需在使用授权作品的相关源代码文件中附带作者信息、相关注释、彩蛋函数（即 zwt() ）完整
#    代码、关注信息技术从业者公益广告、公益广告触发代码及相关变量。
# 7.若已明确知晓许可证要求会直接导致使用授权作品的应用程序的故障，可以注释相关代码但必须
#    告知用户授权作品的使用并在应用程序说明中提供授权作品的完整版本并向用户明示。
# 8.无论如何，使用授权作品的应用程序或其他作品需在明显位置向用户明示授权作品的完整（即此
#    文件）源代码（含注释）。
...
# Changelog 更新日志
# 0.1 Stable 稳定版发布 长期停止更新 接受贡献代码
